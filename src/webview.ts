...
function getWebviewScript(): string {
  ...
  lines.push("      html += '<button class=\"btn btn-primary btn-sm\" data-action=\"manual-model\" data-id=\"' + attr(provider.id) + '\">+模型</button>'; ");
  lines.push("      html += '<button class=\"btn btn-primary btn-sm\" data-action=\"preset-models\" data-id=\"' + attr(provider.id) + '\">导入预设</button>'; ");
  ...
  lines.push("          html += '<button class=\"btn btn-danger btn-xs\" data-action=\"delete-model\" data-id=\"' + attr(provider.id) + '\" data-model-id=\"' + attr(model.id) + '\">删</button>'; ");
  lines.push("          html += '</div>';");
  lines.push("        }");
  lines.push("      } else {");
  ...
  lines.push("        var modelId = button.getAttribute('data-model-id');");
  ...
  lines.push("    var key = String(choice || '').trim().toLowerCase();");
  lines.push("    var openai = [");
  lines.push("      preset('gpt-5.5', 'GPT-5.5', 1050000, ['none', 'low', 'medium', 'high', 'xhigh']),");
  lines.push("      preset('gpt-5.4', 'GPT-5.4', 1050000, ['none', 'low', 'medium', 'high', 'xhigh']),");
  lines.push("      preset('gpt-5.4-mini', 'GPT-5.4 Mini', 1050000, ['none', 'low', 'medium', 'high', 'xhigh']),");
  lines.push("      preset('gpt-5.4-nano', 'GPT-5.4 Nano', 1050000, ['none', 'low', 'medium', 'high']),");
  lines.push("      preset('gpt-5.3-codex', 'GPT-5.3 Codex', 1050000, ['none', 'low', 'medium', 'high', 'xhigh'])");
  lines.push("    ];");
  lines.push("    var gemini = [");
  lines.push("      preset('gemini-3-pro-preview', 'Gemini 3 Pro Preview', 1000000, ['low', 'high']),");
  lines.push("      preset('gemini-3-flash-preview', 'Gemini 3 Flash Preview', 1000000, ['low', 'high']),");
  lines.push("      preset('gemini-2.5-pro', 'Gemini 2.5 Pro', 1000000, ['low', 'high']),");
  lines.push("      preset('gemini-2.5-flash', 'Gemini 2.5 Flash', 1000000, ['low', 'high']),");
  lines.push("      preset('gemini-2.5-flash-lite', 'Gemini 2.5 Flash Lite', 1000000, ['low', 'high'])");
  lines.push("    ];");
  lines.push("    var claude = [");
  lines.push("      preset('claude-opus-4-1-20250805', 'Claude Opus 4.1', 200000, ['low', 'medium', 'high', 'max']),");
  lines.push("      preset('claude-opus-4-20250514', 'Claude Opus 4', 200000, ['low', 'medium', 'high', 'max']),");
  lines.push("      preset('claude-sonnet-4-20250514', 'Claude Sonnet 4', 200000, ['low', 'medium', 'high', 'max']),");
  lines.push("      preset('claude-3-7-sonnet-20250219', 'Claude 3.7 Sonnet', 200000, ['low', 'medium', 'high', 'max']),");
  lines.push("      preset('claude-3-5-sonnet-20241022', 'Claude 3.5 Sonnet', 200000, ['low', 'medium', 'high'])");
  lines.push("    ];");
  lines.push("    var deepseek = [");
  lines.push("      preset('deepseek-chat', 'DeepSeek Chat', 128000, ['high', 'max']),");
  lines.push("      preset('deepseek-reasoner', 'DeepSeek Reasoner', 128000, ['high', 'max']),");
  lines.push("      preset('deepseek-v4-pro', 'DeepSeek V4 Pro', 128000, ['high', 'max']),");
  lines.push("      preset('deepseek-v4-flash', 'DeepSeek V4 Flash', 128000, ['high', 'max'])");
  lines.push("    ];");
  lines.push("    if (key === 'openai' || key === 'gpt') return openai;");
  lines.push("    if (key === 'gemini' || key === 'google') return gemini;");
  lines.push("    if (key === 'claude' || key === 'anthropic') return claude;");
  lines.push("    if (key === 'deepseek') return deepseek;");
  lines.push("    if (key === 'all' || key === '全部') return openai.concat(gemini, claude, deepseek);");
  lines.push("    return [];");
  lines.push("  }");
  lines.push("");
  lines.push("  function preset(id, displayName, maxInputTokens, reasoningOptions) {");
  lines.push("    return { id: id, displayName: displayName, maxInputTokens: maxInputTokens, reasoningEffortOptions: reasoningOptions || [] };");
  lines.push("  }");
  lines.push("");
  lines.push("  function deleteModel(providerId, modelId) {");
  lines.push("    if (!providerId || !modelId) return;");
  lines.push("    var model = null;");
  lines.push("    for (var i = 0; i < models.length; i++) if (models[i].id === modelId) model = models[i];");
